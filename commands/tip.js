...
module.exports = async (msg) => {
    ...
    if (msg.text.length === 3) {
        ...
        pool = false;
        from = msg.sender;
        to = msg.text[1].replace(">", ""); //Turn <prefix@ into <@.
        amount = msg.text[2];
    //Tip from a pool.
        ...
        if (Object.keys(pools).indexOf(from) === -1) {
            msg.obj.reply("That pool or users doesn't exist.");
            return;
        }
    ...
    //Else...
        ...
        //Parse amount into a BN, yet make sure not < 1 satoshi.
        amount = BN(BN(amount).toFixed(process.settings.coin.decimals));
    }

    //If this is not a valid user or pool.
    if (
        (
            (to.substr(0, 2) !== "<@") ||
